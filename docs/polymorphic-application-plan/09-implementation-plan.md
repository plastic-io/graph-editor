...
| PB-052 browser capability host | `packages/Node/NodeComponent.vue:34-72` (stores + `transact`), `packages/Orchestrator/schedulerWorker.ts:49` (`new Scheduler`), `src/main.ts:7-8` (`window.OpenAI`), `packages/Utils/main.ts:12-26` (`loadScripts`) | ambient authority | pass `host.ui` only; worker gets `host` from grants; remove `window.OpenAI`; replace `properties.scripts` with SRI-pinned `deps` | `HostBindings` | integration: template cannot reach stores | **breaking for templates using stores** → migration flag `legacyTemplateAccess` per graph, off by default in M3 |
...
| M3 | W5, W6, W10, W11 (contracts + immutable publish), W9 basic hybrid, W7 isolated-vm, first journey (W15) | IaC, replay/shadow, Rust | M2, S-1, S-2 | commit→activate→hybrid execution→observations→contract validation→one journey; capability host in both domains | backend + editor + runtime | 6–10 wk | breaking template access; isolate perf | traces §6.1, §6.3, §6.4 executable as tests; journey green in CI | activation pointer revert; `legacyTemplateAccess` flag |
