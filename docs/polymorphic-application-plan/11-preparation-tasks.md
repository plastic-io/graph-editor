...
| 3 | **Run spikes S-1 (isolated-vm in Lambda) and S-4 (staged validation cost)** before trusting the M1/M3 estimates. | **done 2026-09-20** — both exit criteria met | `spikes/S-1-report.md`, `spikes/S-4-report.md`, `spikes/s1/`, `spikes/s4-staging-cost.spec.ts` |
...
| 5 | **Unblock parallel work:** publish `@plastic-io/graph-crdt` to npm (PB-120), configure the Auth0 API with the §4.4.5 scope names and one test agent client, create a CI deploy role with OIDC (PB-125); a `staging` stage and a `pio-test-` AWS environment before M3. | **prepared 2026-09-20, blocked on credentials** (npm scope owner, Auth0 admin, AWS admin for the IAM stack) | `12-task5-setup.md`; `graph-server/infra/github-oidc-deploy-role.yaml` (validated) + `.github/workflows/deploy.yml` |
